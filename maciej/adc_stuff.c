...
int i2c_write(uint32_t base_addr, uint8_t dev_addr, uint8_t data_addr, uint8_t *data, int length)
{
	int i;
	pdma_write(base_addr + I2C_REG_TXR, dev_addr | 0);
	pdma_write(base_addr + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	i2c_wait_busy(base_addr);
	
	pdma_write(base_addr + I2C_REG_TXR, data_addr);
	pdma_write(base_addr + I2C_REG_CR, I2C_CR_WR);
	i2c_wait_busy(base_addr);

	for (i=0;i<length-1;i++)
	{
		pdma_write(base_addr + I2C_REG_TXR, data[i]);
		pdma_write(base_addr + I2C_REG_CR, I2C_CR_WR);
		i2c_wait_busy(base_addr);
	}	

	pdma_write(base_addr + I2C_REG_TXR, data[i]);
	pdma_write(base_addr + I2C_REG_CR, I2C_CR_WR | I2C_CR_ACK | I2C_CR_STO);
	i2c_wait_busy(base_addr);
}

int i2c_read(uint32_t base_addr, uint8_t dev_addr, uint8_t data_addr, uint8_t *data, int length)
{
	int i;
	pdma_write(base_addr + I2C_REG_TXR, dev_addr | 0);
	pdma_write(base_addr + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	...
	pdma_write(base_addr + I2C_REG_TXR, dev_addr | 1);
	pdma_write(base_addr + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	i2c_wait_busy(base_addr);

	for (i=0;i<length-1;i++)
	{
		pdma_write(base_addr + I2C_REG_CR, I2C_CR_RD);
		data[i] = pdma_read(base_addr + I2C_REG_RXR);
		i2c_wait_busy(base_addr);
	}	

	pdma_write(base_addr + I2C_REG_CR, I2C_CR_RD | I2C_CR_ACK);
	data[i] = pdma_read(base_addr + I2C_REG_RXR);
	i2c_wait_busy(base_addr);


	pdma_write(base_addr + I2C_REG_CR, I2C_CR_STO);
}



void i2c_mem_write(uint8_t dev_addr, uint16_t data_addr, uint8_t *data, int length)
{
	int i;
	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, dev_addr | 0);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	i2c_wait_busy(BASE_I2C_IDMEM);
	
	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, data_addr >> 8);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_WR);
	i2c_wait_busy(BASE_I2C_IDMEM);

	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, data_addr & 0x00FF);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_WR);
	i2c_wait_busy(BASE_I2C_IDMEM);

	for (i=0;i<length-1;i++)
	{
		pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, data[i]);
		pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_WR);
		i2c_wait_busy(BASE_I2C_IDMEM);
	}	

	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, data[i]);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_WR | I2C_CR_ACK | I2C_CR_STO);
	i2c_wait_busy(BASE_I2C_IDMEM);

}


void i2c_mem_read(uint8_t dev_addr, uint16_t data_addr, uint8_t *data, int length)
{
	int i;
	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, dev_addr | 0);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	...
	pdma_write(BASE_I2C_IDMEM + I2C_REG_TXR, dev_addr | 1);
	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_STA | I2C_CR_WR);
	i2c_wait_busy(BASE_I2C_IDMEM);

	for (i=0;i<length-1;i++)
	{
		pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_RD);
		data[i] = pdma_read(BASE_I2C_IDMEM + I2C_REG_RXR);
		i2c_wait_busy(BASE_I2C_IDMEM);
	}	

	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_RD | I2C_CR_ACK);
	data[i] = pdma_read(BASE_I2C_IDMEM + I2C_REG_RXR);
	i2c_wait_busy(BASE_I2C_IDMEM);


	pdma_write(BASE_I2C_IDMEM + I2C_REG_CR, I2C_CR_STO);
}
